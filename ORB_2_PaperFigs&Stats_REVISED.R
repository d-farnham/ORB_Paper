...
load(file = 'Processed_Data/QA.RData')

# embed the laging and leading values of EWD and QA_700_anom
RIP_EWD_QA = merge(merge(CPC_mod_cell_RIP,EWD, by = c("date","season")),QA,by = c("date","season")) %>%
                          dplyr::select(c(date, season, RIP, EWD, QA_700_anom)) %>%
                          dplyr::mutate(`EWD_-1` = lag(EWD, 1), `QA_anom_-1` = lag(QA_700_anom, 1),
                                        `EWD_-2` = lag(EWD, 2), `QA_anom_-2` = lag(QA_700_anom, 2),
                                        `EWD_-3` = lag(EWD, 3), `QA_anom_-3` = lag(QA_700_anom, 3),
                                        `EWD_-4` = lag(EWD, 4), `QA_anom_-4` = lag(QA_700_anom, 4),
                                        `EWD_-5` = lag(EWD, 5), `QA_anom_-5` = lag(QA_700_anom, 5),
                                        `EWD_00` = EWD,         `QA_anom_00` = QA_700_anom,
                                        `EWD_01` = lead(EWD, 1), `QA_anom_01` = lead(QA_700_anom, 1),
                                        `EWD_02` = lead(EWD, 2), `QA_anom_02` = lead(QA_700_anom, 2),
                                        `EWD_03` = lead(EWD, 3), `QA_anom_03` = lead(QA_700_anom, 3)) %>%
                          dplyr::group_by(season) %>%
                          dplyr::mutate(EWD_mean = mean(EWD, na.rm = TRUE),
                                        QA_anom_mean = mean(QA_700_anom, na.rm = TRUE)) %>%
                          dplyr::select(-EWD, -QA_700_anom)

# only select the RIP days
RIP_EWD_QA_long = melt(RIP_EWD_QA, id.vars = c("season", "date", "RIP", "EWD_mean", "QA_anom_mean")) %>%
                        dplyr::filter(RIP == 1)

# compute the percentiles of interest
RIP_EWD_QA_time = RIP_EWD_QA_long %>% dplyr::group_by(season, variable) %>%
                                        dplyr::summarise(median = median(value, na.rm = TRUE),
                                                         ...
                                                         QA_anom_mean = mean(QA_anom_mean))

# separate 'variable' into 'EWD' or 'MHC' and the lag number
# load function to efficiently do this
source("R/SubstrRightLeft.R")

RIP_EWD_QA_time = RIP_EWD_QA_time %>% dplyr::mutate(lag = as.numeric(substrRight(as.character(variable),2)),
                                                      var = substrLeft(as.character(variable),3))

EWD_time =
  ggplot(RIP_EWD_QA_time[RIP_EWD_QA_time$season == "MAM" &
                           RIP_EWD_QA_time$var == "EWD"	, ]) +
...
# Figure 7
pdf('Final figures/REVISED/Figure_7.pdf', width = 12, height = 3)
grid.arrange(EWD_QA_location,
             EWD_time,
             QA_anom_time,
             ncol = 3)
dev.off()
...
RIP_EWD_QA_obs = merge(merge(CPC_mod_cell_RIP,EWD, by = c("date","season")),QA,by = c("date","season")) %>%
                              dplyr::select(c(date, season, RIP, EWD, QA_700_anom)) %>%
                              dplyr::mutate(model = "OBS")

load(file = 'Processed_Data/mod_RIP.RData')
load(file = 'Processed_Data/EWD_mod.RData')
load(file = 'Processed_Data/QA_mod.RData')

# embed the laging and leading values of EWD and MHC_anom for the GCM
RIP_EWD_QA_mod = merge(merge(mod_RIP,EWD_mod, by = c("date","season","model")),QA_mod,by = c("date","season","model")) %>%
                              dplyr::select(c(date, season, model, RIP, EWD, QA_700_anom))

# combine the observed and GCM
RIP_EWD_QA_all = rbind(RIP_EWD_QA_obs, RIP_EWD_QA_mod)

# mark whether EWD is positive
RIP_EWD_QA_all = RIP_EWD_QA_all %>% dplyr::mutate(EWD.positive = ifelse(EWD > 0, 1, 0))

save(RIP_EWD_QA_all, file = 'Processed_Data/RIP_EWD_QA_all.RData')

# check the correlations for MHC and EWD
cors = RIP_EWD_QA_all %>% dplyr::group_by(season,model) %>%
                           dplyr::summarise(pearson = cor(EWD, QA_700_anom, use = "complete.obs", method = "pearson"),
                                            spearman = cor(EWD, QA_700_anom, use = "complete.obs", method = "spearman"),
                                            pearson_lag = cor(EWD, lag(QA_700_anom,1), use = "complete.obs", method = "pearson"),
                                            spearman_lag = cor(EWD, lag(QA_700_anom,1), use = "complete.obs", method = "spearman"))

ggplot(RIP_EWD_QA_all) +
  geom_point(aes(EWD, QA_700_anom)) +
  facet_wrap(~model)
...
assign("EWD_cdf_plot",
       ggplot() + 
         stat_ecdf(data = RIP_EWD_QA_all[RIP_EWD_QA_all$season %in% c("MAM") & RIP_EWD_QA_all$model != "OBS",], 
...
assign("M_cdf_plot",
       ggplot() + 
         stat_ecdf(data = RIP_EWD_QA_all[RIP_EWD_QA_all$season %in% c("MAM") & RIP_EWD_QA_all$model != "OBS",], 
                   aes(x = QA_700_anom, group = model), linetype = "dashed") +
         stat_ecdf(data = RIP_EWD_QA_all[RIP_EWD_QA_all$season %in% c("MAM") & RIP_EWD_QA_all$model == "OBS",], 
                   aes(x = QA_700_anom)) +
         theme(legend.position="none") +
         theme(legend.position="none") +
...
pdf('Final figures/REVISED/Figure_8.pdf', width = 11, height = 3)
...
tail_dependence_data = data.frame(date = RIP_EWD_QA_all$date,
                                  season = RIP_EWD_QA_all$season,
                                  model = RIP_EWD_QA_all$model,
                                  EWD = RIP_EWD_QA_all$EWD,
                                  M_anom = RIP_EWD_QA_all$QA_700_anom)
...
for(llag in 1:lag.max){
  tdc.EWD[[llag]] = data.frame(model = unique(tail_dependence_data$model),
                               ...
                               P_0.95 = NA,
                               P_0.99 = NA)
  tdc.M[[llag]] = data.frame(model = unique(tail_dependence_data$model),
                               P_0.9 = NA,
                               P_0.95 = NA,
                               P_0.99 = NA)
}
...
for(llag in 1:lag.max){
  ...
  for(mmodel in 1:length(unique(tmp$model))){
    for(pper in 1:(ncol(tdc.EWD[[llag]])-1)){
      ...
      tdc.M[[llag]][mmodel,pper+1] = tdc(tmp$M_anom[tmp$model == unique(tmp$model)[mmodel]], 
                                           tmp$M_anom_lag[tmp$model == unique(tmp$model)[mmodel]], 
                                           p = as.numeric(strsplit(names(tdc.M[[llag]])[pper+1], "_")[[1]][2]), 
                                           upper = TRUE)
    }
...
tdc.M_long = melt(tdc.M) %>% setnames("L1","lag") %>%
                                 dplyr::mutate(variable = gsub("_", " = ", as.character(variable)))

M_tail_persist = 
  ggplot() +
  geom_line(data = tdc.M_long[tdc.M_long$model != "OBS",],
            aes(lag, value*100, group = model), linetype = "dashed") +
  geom_line(data = tdc.M_long[tdc.M_long$model == "OBS",],
            aes(lag, value*100)) +
  facet_wrap(~variable) + 
  theme_bw() +
  scale_x_continuous(breaks = c(1:10)) +
  labs(y = expression(Pr(F(M[t])~ ">" ~ P ~ "|" ~ F(M[t-lag]) ~ ">" ~ P)))


pdf('Final figures/REVISED/Figure_9.pdf', width = 10, height = 6)
...
########################################
rm(list = ls())
load(file = 'Processed_Data/RIP_EWD_QA_all.RData')

# rename the model
RIP_EWD_QA_all = RIP_EWD_QA_all %>% dplyr::mutate(model = ifelse(model != "OBS", paste0("GCM ", model), model))

alpha = 1
locfit_fits_MAM = list()
for(mm in 1:length(unique(RIP_EWD_QA_all$model))){
  
  mmodel = unique(RIP_EWD_QA_all$model)[mm]
  locfit_data = RIP_EWD_QA_all %>% dplyr::filter(model == mmodel &
                                                  season == "MAM")
  
  locfit_fits_MAM[[mm]] <- locfit(RIP ~ EWD + QA_700_anom,
                                  data = locfit_data,
                                  family="binomial",
                                  alpha = alpha)
}
names(locfit_fits_MAM) = unique(RIP_EWD_QA_all$model)


# now do the same for pooled GCM ensemble members
locfit_data = RIP_EWD_QA_all %>% dplyr::filter(model != "OBS" &
                                                season == "MAM")

locfit_fits_ens_MAM <- locfit(RIP ~ EWD + QA_700_anom,
                              data = locfit_data,
                              family="binomial",
                              alpha = alpha)
...
grid.size.M_anom = 0.0005
grid.size.EWD = 30
reg_grid_MAM = 
  expand.grid(QA_700_anom = seq(-0.004,0.004, by = grid.size.M_anom), 
...
bins_low_count0 = RIP_EWD_QA_all %>% dplyr::mutate(QA_700_anom = grid.size.M_anom*round(QA_700_anom/(grid.size.M_anom)),
                                                    EWD = grid.size.EWD*(round(EWD/grid.size.EWD))) %>%
                                      dplyr::group_by(model, season, QA_700_anom, EWD) %>%
                                      dplyr::summarise(count = length(RIP))


all_bins = data.frame(expand.grid(QA_700_anom = unique(reg_grid_MAM$QA_700_anom), 
                                  EWD = unique(reg_grid_MAM$EWD),
                                  model = unique(RIP_EWD_QA_all$model),
                                  season = unique(RIP_EWD_QA_all$season)),
                                  count = 0)


all_bins_count1 = merge(bins_low_count0, all_bins, by = c("model", "QA_700_anom",
                                                          "EWD", "season"), all = TRUE) %>%
                        dplyr::mutate(count = ifelse(is.na(count.x), count.y, count.x),
                                      variable = factor(model))

all_bins_count_ens = all_bins_count1 %>% dplyr::group_by(QA_700_anom, EWD, season) %>%
                                         dplyr::summarise(count = min(count)) %>%
                                         dplyr::mutate(variable = "ENS - OBS") %>%
                                         dplyr::ungroup()

all_bins_count1 = all_bins_count1 %>% dplyr::select(c(QA_700_anom, EWD, season, count, variable))

all_bins_count = rbind(all_bins_count1, all_bins_count_ens)

for(mm in 1:length(unique(RIP_EWD_QA_all$model))){
  reg_grid_MAM[,mm+2] = predict(locfit_fits_MAM[[mm]], newdata = reg_grid_MAM)
  colnames(reg_grid_MAM)[mm+2] = unique(RIP_EWD_QA_all$model)[mm]
}
reg_grid_MAM[,6] = predict(locfit_fits_ens_MAM, newdata = reg_grid_MAM)
colnames(reg_grid_MAM)[6] = "ENS"

reg_grid_MAM = reg_grid_MAM %>% dplyr::mutate("ENS - OBS" = ENS - OBS) %>%
  dplyr::select(-c(ENS))

reg_grid_MAM_long = melt(reg_grid_MAM, id.vars = c("QA_700_anom","EWD"))
...
obs_mod_plot = 
  ...
  geom_tile(data = all_bins_count[all_bins_count$count < min.obs &
                                    ...
                                    all_bins_count$variable %in% unique(reg_grid_MAM_long$variable)[1:3],],
            aes(x = QA_700_anom,
                y = EWD),
            fill = "grey") +
#  coord_cartesian(xlim = c(-5.3,5.3), ylim = c(-235,235)) +
  labs(x = "M Index", y = "EWD Index") +
  theme_bw() +
  theme(legend.position = "bottom") +
...
pdf('Final figures/REVISED/Figure_S8.pdf', height = 3, width = 9)
...
# !!! NOW CHECK THE SIMULATION MODEL TO REPRODUCE THE OBSERVED DATA !!!
n.sims = 10
...
# !!!! TESTING: LOOK AT THE SCATTERPLOT OF RIP VS. RIP_SIM BY YEAR -- BUT NO RELATIONSHIP SOUHLD BE EXPECTED HERE SINCE THE CLOCKS DO NOT LINE UP!!!
...
# fit the model on the observations
RIP_EWD_QA_mod = glm(RIP ~ QA_700 : EWD.positive + factor(RIP_lag1) : EWD, 
                      data = RIP_EWD_QA_all_MAM[RIP_EWD_QA_all_MAM$model == "OBS",], family = binomial(link = "logit"))
