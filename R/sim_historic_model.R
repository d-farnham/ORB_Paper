...
REP_test4_sim = data.frame(date = testing_data4$date,
                          REP = testing_data4$REP,
                          model = 4,
                          t(data.frame(rstan::extract(stan_fit_4, pars = "y_test_sim"))))
REP_test1_sim = data.frame(date = testing_data1$date,
                           REP = testing_data1$REP,
                           model = 1,
                           t(data.frame(rstan::extract(stan_fit_1, pars = "y_test_sim"))))

REP_test_sim = rbind(REP_test1_sim, REP_test4_sim)
...
REP_sim_yearly_training_plot =  REP_sim_yearly_training %>% dplyr::filter(year > (1950 + (smoothing_window/2 - 1)) &
...
REP_obs = dat_include %>% dplyr::filter(model == "obs") %>%
                          dplyr::group_by(year = lubridate::year(date)) %>%
                          dplyr::summarise(REP = sum(REP))

# need to normalize by: 1000
iter*chains/2/thin
...
REP_mod_obs_compare_2 =
  ...
  stat_bin(data = REP_test_sim_yearly0[REP_test_sim_yearly0$model == 1,],
           ...
           breaks = bin_breaks) +
  stat_bin(data = REP_test_sim_yearly0[REP_test_sim_yearly0$model == 4,],
           aes(x = REP_sim, y =..count../(1000), group = model),
           geom = "step", linetype = "dashed", size = line.width,
           breaks = bin_breaks) +
  stat_bin(data = REP_test_sim_yearly0[REP_test_sim_yearly0$model == 1,],
...
# now check the REP lag 1 correlation by simulation
REP_sim_daily = melt(REP_test_sim, id.vars = c("date","model","REP")) %>% dplyr::group_by(variable) %>%
  dplyr::mutate(value_lag1 = lag(value, 1),
                REP_lag1 = lag(REP, 1))

# create a three way contingency table for the simulations
REP_sim_daily_table = as.data.frame(ftable(REP_sim_daily %>% dplyr::select(value, value_lag1, variable, model)))

for(ii in 1:(nrow(REP_sim_daily_table)/4)){
  REP_mod_persist0 = data.frame(variable = REP_sim_daily_table$variable[ii*4],
                                set = paste0("ens # ",REP_sim_daily_table$model[ii*4]),
...
REP_obs_daily_table = as.data.frame(ftable(training_data %>% dplyr::select(REP, REP_lag1)))
...
pdf('Final figures/Figure_8.pdf', width = 12, height = 2.75)
